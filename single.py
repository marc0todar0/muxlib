...
def get_single(url: str, FOLDER: str = ".", EXT:str = "mp3") -> str:
    if "playlist" in url:
        raise Exception("get_single cannot download an album/playlist. Please use get_album/get_playlist")
    ...
    title = re.sub(r'\s*\((Visual|Official Video|Lyric Video|Audio|Official Audio)\)\s*', '', title, flags=re.IGNORECASE).strip()
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ydl.download([url])
    file_size_bytes = os.path.getsize(f"{final_path}.{EXT}")
    file_size_mb = file_size_bytes / (1024 * 1024)
    desc = f"""File salvato come: {final_path}.{EXT}
